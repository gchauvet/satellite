...
static int check_tmp_file(arg_data *args)
{
    ...
    if (pid < 0)
        return -1;
    sprintf(buff, "/tmp/%d.deimos_up", pid);
...
static int child(arg_data *args, home_data *data, uid_t uid, gid_t gid)
{
    ...
    if (args->vers == true) {
        log_error("Deimos (Hall Project) " DEIMOS_VERSION_STRING);
...
int main(int argc, char *argv[])
{
    ...
    if (strcmp(argv[0], args->procname) != 0) {
        ...
        tmp = strchr(argv[0], '/');
        if (tmp == NULL) {
            log_error("DEIMOS re-exec requires execution with an absolute or relative path");
            return 1;
        }
        ...
        log_error("Cannot execute DEIMOS executor process (%s)", filename);
        return 1;
    }
